...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %   it has fields ret.input_to_hid and ret.hid_to_class. 
  ...
  % Before we can calculate the gradient, we need to calculate a variety of intermediate values, like the state of the hidden units.
  hid_input = model.input_to_hid * data.inputs; % input to the hidden units, i.e. before the logistic. size: <number of hidden units> by <number of data cases>
  hid_output = logistic(hid_input); % output of the hidden units, i.e. after the logistic. size: <number of hidden units> by <number of data cases>
  class_input = model.hid_to_class * hid_output; % input to the components of the softmax. size: <number of classes, i.e. 10> by <number of data cases>
  class_normalizer = log_sum_exp_over_rows(class_input); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
  ...
  % first compute the gradient for w.r.t. the hidden -> softmax weights
  diff = class_prob - data.targets; % <num classes x num cases>
  % hid_output <hidden x cases>
  before_avg = (diff * hid_output');
  after_avg = before_avg ./ (size(data.targets,2));
  ret.hid_to_class = after_avg + (model.hid_to_class * wd_coefficient); % <classes x hidden>
  
  % next, compute the gradient for the input -> hidden weights
  % model.hid to class = <num classes x num hidden units>
  weight_factor = diff' * model.hid_to_class; % = <num cases x num hidden units>
  hidden_factor = hid_output .* (1 - hid_output); % <num hidden units x num cases>
  almost = hidden_factor .* weight_factor'; % = <num hidden units x num cases>
  % data.inputs = <num inputs x num cases>
  before_avg = almost * data.inputs'; % = <num hidden units x num inputs>
  after_avg = before_avg ./ (size(data.targets,2));
  ret.input_to_hid = after_avg + (model.input_to_hid * wd_coefficient);
