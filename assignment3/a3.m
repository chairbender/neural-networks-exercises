...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %we use this term in a few places
  diff = class_prob - data.targets; % <num classes x num cases>
  
  % first compute the gradient for w.r.t. the hidden -> softmax weights
  ret.hid_to_class = (1 / m) * (diff * hid_output') + (model.hid_to_class * wd_coefficient);
  
  % next, compute the gradient for the input -> hidden weights
  before_reg = (diff' * model.hid_to_class)' .* hid_output .* (1 - hid_output) * data.inputs' * (1 / m);
  ret.input_to_hid = before_reg + (model.input_to_hid * wd_coefficient);
